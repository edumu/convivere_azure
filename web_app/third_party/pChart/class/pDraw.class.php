 ...
 class pDraw
  {
   ...
   function scaleComputeY($Values=array(),$Option=array(),$ReturnOnly0Height=FALSE)
    {
     ...
     if ( $Data["Orientation"] == SCALE_POS_LEFTRIGHT )
      {
       ...
       $Step        = ($ScaleHeight ===0)?$Height:$Height / $ScaleHeight;
       
       $dataArr = is_array($Values) ? $Values : array($Values);
       if ( $ReturnOnly0Height  )
        { foreach($dataArr as $Value) { if ( $Value == VOID ) { $Result[] = VOID; } else { $Result[] = $Step * $Value; } } }
       else 
        { foreach($dataArr as $Value) { $Result[] = $this->GraphAreaY2 - $Data["Axis"][$AxisID]["Margin"] - ($Step * ($Value-$Data["Axis"][$AxisID]["ScaleMin"])); } } 
      }
     else
      {
